...
StatusCode TrackObject::fillTrackContainer(std::string sys_name)
{
  ...
  ASG_CHECK_SA( "SusySkimMaker", selTool.setProperty("CutLevel", "LoosePrimary" ));
  ASG_CHECK_SA( "SusySkimMaker", selTool.setProperty("maxZ0SinTheta", 3.) );
  ASG_CHECK_SA( "SusySkimMaker", selTool.setProperty("minPt", 1.) );
  ASG_CHECK_SA( "SusySkimMaker", selTool.initialize() );

  for( const auto& track : *pixelTracklets ){

    if ( !selTool.accept(track) ) continue;
    ...
    m_event->retrieve( primVertex, "PrimaryVertices" );
