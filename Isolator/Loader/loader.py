...
def group_leptons_and_tracks(leptons, tracks):
    ...
    good_track_hits = [i + j >= 7 for i, j in zip(
        tracks['nSCTHits'], tracks['nPixHits'])] and (tracks['nIBLHits'] > 0)
    good_track_holes = [i + j <= 2 for i, j in zip(
        tracks['nPixHoles'], tracks['nSCTHoles'])] and (tracks['nPixHoles'] <= 1)
    ...
    # track_labels = ['dR', 'dEta', 'dPhi', 'dd0', 'dz0', 'charge', 'eta', 'pT', 'theta', 'd0', 'z0', 'chiSquared']
    track_labels = ['dR', 'pT']
...
def create_or_load(in_file, save_file_name, overwrite=False):
    ...
    # else, group leptons and tracks and save the data
        ...
        lepton_labels = ['pdgID', 'pT', 'eta', 'phi', 'd0', 'z0', 'ptcone20', 'ptcone30',
                         'ptcone40', 'ptvarcone20', 'ptvarcone30', 'ptvarcone40', 'truth_type']
        ...
        for event_n in range(n_events):
            ...
            if len(leptons) == 0:
                continue
            grouped_leptons, grouped_tracks, track_labels = group_leptons_and_tracks(
                leptons, tracks[event_n])
        ...
        ignore = [0, 12]   # ignore pdgID and truth_type
        lepton_means[ignore] = 0
        lepton_stds[ignore] = 1
        ...
        leptons_with_tracks = {'unnormed_leptons': unnormed_leptons, 'normed_leptons': normed_leptons,
                               'unnormed_tracks': unnormed_tracks, 'normed_tracks': normed_tracks,
                               'lepton_labels': lepton_labels, 'track_labels': track_labels}
